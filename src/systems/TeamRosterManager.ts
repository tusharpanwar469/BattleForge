...
export class TeamRosterManager {
    ...
    addFighter(
        ...
        const errors: string[] = [];
        const normalizedId =
            fighterId.trim().toLowerCase();

        if (!normalizedId) {
            errors.push(
                "Fighter ID cannot be empty."
            );
        ...
        if (!this.registry.has(normalizedId)) {
            errors.push(
                `Fighter "${fighterId}" is not registered.`
            );
        ...
        if (
            team.eliminatedFighters.includes(
                normalizedId
            )
        ) {
            errors.push(
        ...
        if (
            ...
            errors.push(
                `Fighter "${fighterId}" is already on the team roster.`
            );
        ...
        if (
            team.roster.length >= TEAM_MAX_ROSTER
        ) {
            errors.push(
    ...
    removeFighter(
        ...
        if (fighterIndex === -1) {
            errors.push(
                `Fighter "${fighterId}" is not on the team roster.`
            );
