...
export class AuctionSaleManager {
  ...
  settleSale(
    ...
    const auctionValidation = validateAuctionState(auction);
    ...
    if (auction.status !== "sold") {
      return {
        success: false,
        errors: ["Only a sold auction can be settled."]
      };
    }
    ...
    if (finalPrice === null) {
      return {
        success: false,
        errors: ["Sold auction must have a final bid."]
      };
    }
    ...
    if (finalPrice > team.budget) {
      return {
        success: false,
        errors: ["Team cannot afford the final auction price."]
      };
    }

    const rosterResult = this.rosterManager.addFighter(
      team,
      auction.fighterId
    );
